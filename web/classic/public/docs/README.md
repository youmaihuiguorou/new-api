...
欢迎使用 熊猫 提供的 AI 模型 API 路由服务！我们基于统一的 OpenAI API 标准，支持 OpenAI 官方所有模型及市面上其他主流大模型（如 Claude、Gemini、DeepSeek、Qwen、Coze等）。无论调用哪种模型，使用方法都保持一致，只需在参数中修改模型名称为对应的名称即可。
